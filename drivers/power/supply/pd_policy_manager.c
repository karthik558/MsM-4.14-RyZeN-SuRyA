...
static void usbpd_pm_update_cp_status(struct usbpd_pm *pdpm)
{
	...
	ret = power_supply_get_property(pdpm->cp_psy,
			POWER_SUPPLY_PROP_SC_VAC_VOLTAGE, &val);
	if (!ret)
		pdpm->cp.vac_volt = val.intval; 
	...
	ret = power_supply_get_property(pdpm->cp_psy,
			POWER_SUPPLY_PROP_SC_REG_STATUS, &val);
	if (!ret) {
...
static void usbpd_pm_evaluate_src_caps(struct usbpd_pm *pdpm)
{
	...
	for (i = 0; i < 7; i++) {
		pr_err("[SC manager] %d type %d %d %d\n", i, pdpm->pdo[i].type,
...
static int usbpd_pm_fc2_charge_algo(struct usbpd_pm *pdpm)
{
	...
	if (ibus_limit == 0)
		ibus_limit = pm_config.bus_curr_lp_lmt;// + 100
	...
	pr_err(">>>>pps ctrl  %d  %d  %d  %d  %d  %d\n", 
			step_vbat, step_ibat, sw_ctrl_steps, hw_ctrl_steps, steps);
...
static int usbpd_pm_sm(struct usbpd_pm *pdpm)
{
	...
	pr_err(">>>>>vbus_vol %d    vbat_vol %d   vac %d\n", pdpm->cp.vbus_volt, pdpm->cp.vbat_volt, pdpm->cp.vac_volt);
	...
	case PD_PM_STATE_FC2_ENTRY_2:
		...
		if (pdpm->cp.vbus_error_low){//(pdpm->cp.vbus_volt < (pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP - 50)) {
			...
			pr_err("request_voltage:%d, request_current:%d\n",
					pdpm->request_voltage, pdpm->request_current);
		} else if (pdpm->cp.vbus_error_high){//(pdpm->cp.vbus_volt > (pdpm->cp.vbat_volt * 2 + BUS_VOLT_INIT_UP + 50)) {
			tune_vbus_retry++;
			pdpm->request_voltage -= 20;
			usbpd_select_pdo(pdpm->pd, pdpm->apdo_selected_pdo,
						pdpm->request_voltage * 1000,
...
static void usbpd_pm_workfunc(struct work_struct *work)
{
	...
	usbpd_update_ibat_curr(__pdpm);
	usb_psy_pd_active_update(__pdpm);

	//pr_err(">>>>>>work cycle pd_active : %d  %d  %d\n", __pdpm->pd_active ,last_state, __pdpm->cp.charge_enabled);

	if (!usbpd_pm_sm(__pdpm) && __pdpm->pd_active)
		schedule_delayed_work(&__pdpm->pm_work,
				msecs_to_jiffies(PM_WORK_RUN_INTERVAL));
	if(__pdpm->pd_active == 0)
	{
		usbpd_pm_disconnect(__pdpm);
	}
...
static void usb_psy_change_work(struct work_struct *work)
{
	...
	pr_err("[SC manager] >> pd_active %d,  val.intval %d,  active %d\n",
			pdpm->pd_active, val.intval, POWER_SUPPLY_PD_PPS_ACTIVE);
